...
public class AdminBlogController {
    ...
    @PutMapping(value = "/{blogId}/images", consumes = MediaType.MULTIPART_FORM_DATA_VALUE,produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> uploadBlogImage(@PathVariable("blogId") Long id, @RequestParam("file") MultipartFile file) {
        return ResponseEntity.ok(blogService.insertImageByBlogId(id,file));
    }

    @Operation(summary = "Gets images by blog ID",
            description = "Gets images using blog id")
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "OK",
    ...
    @GetMapping(value = "/{blogId}/images", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> getBlogImage(@PathVariable("blogId") Long id) {
